...
function GameConnection::getObfuscatedName(%this, %table, %maxLenMod, %minLen)
{
	...
	if(%this.obfuscatedName !$= "" && %this.lastObfuscation >= %mini.roundStart) //only return the last obfuscation if it was after the last round start
	...
	%a = 0;
	if(%mini.running)
		%a = mFloatLength(%mini.roundStart, 0); //we want to keep the gibberish the same on a per-round basis, but different across rounds, so this is a nice solution!
